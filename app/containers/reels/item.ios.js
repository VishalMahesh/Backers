...
const VideoItem = ({ source, paused, onEnd, back, onpay, oncomment, onoption, user, caption }) => {
    ...
    return <View style={{ height: DEVICE_HEIGHT - STATUSBAR_HEIGHT, width: Layout.width, backgroundColor: Colors.black }}>
        <LinearGradient colors={gradientArr} style={[{ height: offset.top + 40, justifyContent: 'space-between', width: wide, position: 'absolute', top: 0, zIndex: 99, flexDirection: 'row', alignItems: 'flex-end', paddingHorizontal: containerPadding / 2 }]}>
            <IconButtons
                name={Images.back}
                color={Colors.light}
                size={26}
        ...
        <PostActions
            ...
            comment={oncomment}
        />
        <LinearGradient colors={gradientArr2} style={[{ height: offset.bottom + 60, width: wide, position: 'absolute', bottom: offset.bottom + wide * 0.2, zIndex: 99, paddingHorizontal: containerPadding / 2 }, CommonStyles.row]}>
            <Caption
                data={user}
                caption={caption}
            />
